...
// Default skills to fallback on if translation fails
const defaultSkills = {
  languages: ["Persian (Native)", "English (B1)", "German (B1)"],
  frameworks: [
    "Technical Training",
    "Tactical Development",
    "Match Preparation",
    "Youth Training Methods",
    "Small-Sided Games",
  ],
  databases: [
    "Sports Psychology",
...
const Skills: React.FC = () => {
  ...
  // Helper function to safely get translated skills
  const getTranslatedSkills = (key: string): string[] => {
    try {
      const translatedItems = t(`skills.items.${key}`, { returnObjects: true });
      if (Array.isArray(translatedItems)) {
        return translatedItems as string[];
      }
      // If translation returns an object but not an array, fallback to default
      return defaultSkills[key as keyof typeof defaultSkills];
    } catch (error) {
      console.error(`Error fetching translated skills for ${key}:`, error);
      return defaultSkills[key as keyof typeof defaultSkills];
    }
  };
            ...
            {skillCategories.map((category, index: number) => (
              ...
              >
                ...
                <div className="space-y-3">
                  {getTranslatedSkills(category.key).map(
                    (skill: string, skillIndex: number) => (
                      ...
                      >
                        <span className="relative text-center px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 rounded-lg transition-all duration-200 group-hover/skill:bg-primary-50 dark:group-hover/skill:bg-primary-900/20 group-hover/skill:text-primary-700 dark:group-hover/skill:text-primary-300 cursor-default">
                          {skill}
